...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type="train"):
        ...
        elif file_type == "test1":
            file_path = args.test1_data_file
        elif file_type == "test2":
            file_path = args.test2_data_file
        elif file_type == "part":
    ...
    def apply_smote(self):
        print("train_dataset: ", len(self.examples))

        entries = []
        train_features = []
        train_targets = []
        for entry in self.examples:
            train_features.append(entry.input_ids)
            train_targets.append(entry.label)
        train_features = np.array(train_features)
        train_targets = np.array(train_targets)
        num_positive = np.sum(train_targets == 1)
        # smote = SMOTE(sampling_strategy={1: num_positive + 10000}, random_state=1000)
        smote = RandomOverSampler(sampling_strategy={1: num_positive + 10000}, random_state=1000)
        features, targets = smote.fit_resample(train_features, train_targets)
        index = 0
        for feature, target in zip(features, targets):
            entries.append(InputFeatures(input_tokens=[], input_ids=feature, label=target, i=index))
            index += 1
...
def train(args, train_dataset, model, tokenizer, test1_dataset, test2_dataset):
    """ Train the model """
    # build dataloader
    ...
    # rho = 0
    # optimizer = SAM(base_optimizer=AdamW, rho=rho, params=optimizer_grouped_parameters, lr=args.learning_rate, eps=args.adam_epsilon)
    # print("rho: ", rho)
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                # loss, _ = model(input_ids=inputs_ids, labels=labels)
                # loss.backward()
                ...
                # if global_step % args.save_steps == 0:
                # if global_step % 50 == 0:
                    test(args, model, tokenizer, test1_dataset, 1)
                    test(args, model, tokenizer, test2_dataset, 2)

                    checkpoint_prefix = 'checkpoint-' + str(idx)
                ...
                # if global_step == 87500:
                #     test(args, model, tokenizer, test1_dataset, 1)
                #     test(args, model, tokenizer, test2_dataset, 2)
                #
                #     checkpoint_prefix = 'checkpoint-step' + str(global_step)
                #     output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))
                #     if not os.path.exists(output_dir):
                #         os.makedirs(output_dir)
                #     model_to_save = model.module if hasattr(model, 'module') else model
...
def test(args, model, tokenizer, test_dataset, test_index, best_threshold=0.5):
    # build dataloader
    test_sampler = SequentialSampler(test_dataset)
    test_dataloader = DataLoader(test_dataset, sampler=test_sampler, batch_size=args.eval_batch_size, num_workers=8)
    ...
    logger.info(f"***** Test{test_index} results *****")
    for key in sorted(result.keys()):
        logger.info("  %s = %s", key, str(round(result[key],4)))
...
def main():
    ...
    parser.add_argument("--test_data_file", default=None, type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")
    parser.add_argument("--test1_data_file", default=None, type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")
    parser.add_argument("--test2_data_file", default=None, type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")
    ...
    # Training
    if args.do_train:
        # if args.part_data_file is None:
        ...
        if args.part_data_file is not None:
            ...
            # train_dataset = TextDataset(tokenizer, args, file_type='train')
            # part_dataset.examples.extend(train_dataset.examples)
            # random.shuffle(part_dataset.examples)
            # part_dataset.examples = sorted(part_dataset.examples, key=lambda x: x.input_ids)
            # train_dataset.examples += part_dataset.examples[:5000]
            train_dataset.examples += part_dataset.examples
        # eval_dataset = TextDataset(tokenizer, args, file_type='eval')
        test1_dataset = TextDataset(tokenizer, args, file_type='test1')
        test2_dataset = TextDataset(tokenizer, args, file_type='test2')
        # train(args, train_dataset, model, tokenizer, eval_dataset)
        train(args, train_dataset, model, tokenizer, test1_dataset, test2_dataset)
    # Evaluation
    results = {}
    if args.do_eval:
