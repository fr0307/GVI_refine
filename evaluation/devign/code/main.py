...
if __name__ == '__main__':
    ...
    # parser.add_argument('--processed_data_path', type=str, help='Path to the processed data.', default='../../devign_storage/shard/reveal')
    ...
    # processed_data_path = os.path.join(args.processed_data_path, 'processed.bin')
    ...
    # dataset.feature_size = dataset.train_examples[0].features.size(1)
    # debug('Feature Size : %d' % dataset.feature_size)
    # dataset.initialize_dataset()

    # positive_entries = [entry for entry in dataset.train_examples if entry.target == 1]
    # rand = random.Random(1000)
    # entra_entries = rand.choices(positive_entries, k=10000)
    # for entry in entra_entries:
    #     dataset.train_examples.append(entry)
    ...
    # rho = 1e-10
    # optim = SAM(base_optimizer=Adam, rho=rho, params=model.parameters(), lr=0.0001, weight_decay=0.001)
    # print("rho: ", rho)
    ...
    if is_test:
        ...
        model.train()
        model = train(model=model, dataset=dataset, max_steps=1400, dev_every=200,
    ...
    # # get_embeddings(model, loss_function,  dataset.initialize_test_batch(),
    # #                                     dataset.get_next_test_batch, model_dir + "/reveal_test_after_ggnn.json")
    #
