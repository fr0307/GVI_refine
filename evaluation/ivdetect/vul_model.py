...
class Vulnerability_backbone(torch.nn.Module):
    ...
    def forward(self, my_data, edge_index, mask=None, x=None):
        if isinstance(my_data, list):
            ...
            # pack for feature 2-5 is done in gen_graphs
            # print(feature_2.data.shape)
            # if feature_2.data.shape[0] == 0:
            #     feature_2.data = feature_2.data.unsqueeze(0)
            feature_2, _ = self.gru_1(feature_2.float())
            feature_2, out_len = pad_packed_sequence(feature_2, batch_first=True)
            ...
            if mask != None:
                messages = x[edge_index[0]] * mask
                feature_vec = scatter(messages, edge_index[1], dim=0, reduce='mean')
    ...
    def forward_first(self, my_data, edge_index, mask=None):
        # Input data format: a list that contains main graph, feature 1, ..., feature 5. The feature 1 is tree
        # structured and features 2-5 are sequences.
        feature_1 = my_data[1]
        feature_2 = my_data[0]
    ...
    def forward_second(self, feature_vec, edge_index, mask=None):
        if mask != None:
            messages = feature_vec[edge_index[0]] * mask
            feature_vec = scatter(messages, edge_index[1], dim=0, reduce='mean')
        for i in range(self.layer_num):
            if i < self.layer_num:
                feature_vec = self.convs[i](feature_vec, edge_index)
...
class backbone_mlp(torch.nn.Module):
    ...
    def forward(self, my_data, edge_index, mask=None, x=None):
        backcone_output = self.backbone(my_data, edge_index, mask, x)
        out = self.mlp(backcone_output)
        out = nn.Softmax(dim=1)(out)
        return out

    def forward_first(self, my_data, edge_index, mask=None):
        backcone_output = self.backbone.forward_first(my_data, edge_index, mask)
        return backcone_output

    def forward_second(self, x, edge_index, mask=None):
        out = self.backbone.forward_second(x, edge_index, mask)
        out = self.mlp(out)
        out = nn.Softmax(dim=1)(out)
        return out
