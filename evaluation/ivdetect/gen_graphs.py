...
if __name__ == '__main__':
    project = "reveal_test"
    data_file = f"processed_data/all_{project}_data.csv"
    ...
    ast_glove_file_dir = f'{os.getcwd()}/glove/ast_vectors.txt'
    ast_tmp_file_dir = f'{os.getcwd()}/glove/ast_gensim.txt'
    pdg_glove_file_dir = f'{os.getcwd()}/glove/pdg_vectors.txt'
    pdg_tmp_file_dir = f'{os.getcwd()}/glove/pdg_gensim.txt'
    ...
    if os.path.isfile(ast_tmp_file_dir):
        os.remove(ast_tmp_file_dir)

    if os.path.isfile(pdg_tmp_file_dir):
        os.remove(pdg_tmp_file_dir)
    ...
    for i in tqdm(range(0, len(data))):
        try:
            ...
            # if loc > 500:
                wrong_cumulate += 1
                print(f'loc {loc} in data {i} is too large, total wrong {wrong_cumulate}')
                continue
            ...
            vul = int(data.at[i, "bug"])
            graph_i.y = torch.tensor([vul], dtype=int)
            graph_i.code = code
            graph_i.lines = data.at[i, "lines"]
            graph_i.stmt_keys = stmt_keys
            ...
            torch.save(graph_i, f'{os.getcwd()}/processed_data/graph_{project}/data_{starting_index}.pt')
            # torch.save(graph_i, f'{os.getcwd()}/processed_data/graph_vgx/data_{starting_index}.pt')
            starting_index += 1
