...
def top_k_accuracy(pred_lines, correct_lines, k=10):
    """
    计算 Top-k Accuracy。

    :param pred_lines: 模型预测的字典，格式为 {sampleid: [pred_1, pred_2, ..., pred_n]}
    :param correct_lines: 真实标签的字典，格式为 {sampleid: {"removed": [labels], "depadd": [labels]}}
    :param k: Top-k 数量
    ...
    for sampleid, pred in pred_lines.items():
        ...
        true_labels = list(true["removed"]) + list(true["depadd"])

        # 如果样本有漏洞（即标签不全是 0）
        if sum(true_labels) > 0:
            total_samples += 1
            # 检查预测的前 k 个结果是否包含真实标签
            top_k_preds = pred[:k]
            if any(label in top_k_preds for label in true_labels):
                correct_count += 1
