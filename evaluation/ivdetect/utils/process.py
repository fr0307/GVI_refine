...
def find_df(out_list, in_list, stmt_num, stmt_list, seq, depth, limit, label, start_time, timeout):
    if time.time() - start_time > timeout:
        # print(f"Timeout exceeded at stmt_num {stmt_num} at depth {depth}. Returning current seq.")
        return seq
    record_out = []
    record_in = []
    if label != 2:
    ...
    if depth < limit:
        for stmt in record_out:
            seq = find_df(out_list, in_list, stmt, stmt_list, seq, depth + 1, limit, 1, start_time, timeout)
        for stmt in record_in:
            seq = find_df(out_list, in_list, stmt, stmt_list, seq, depth + 1, limit, 2, start_time, timeout)
    return seq
...
def generate_feature_5(data, dic, embed_dim, depth=2):
    ...
    for i in tqdm(range(len(dfgs)), desc='fea_5'):
        ...
        for stmt in all_:
            ...
            output_seq = find_df(out_, in_, stmt, feature_set, [], 1, depth, 0, start_time, 100)
            embedding[stmt] = output_seq
        data_output.append(embedding)
    return data_output
