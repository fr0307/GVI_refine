...
import get_full_text_files
import word2vec_code
import word2vec_tokenized
...
class Tokenizer:
    ...
    def process_literal(self, literal):
        ...
        try:
            ...
            print("spelling:\t", literal.spelling)
            # print("kind:\t", literal.cursor.kind)
            print("Error: ", e)
...
def extract_slices(project):
    split_dir = f'./data/{project}/raw_code/'
    parsed = f'./data/{project}/parsed/'
    all_data = []
    ggnn_json_data = json.load(open(f'./data/{project}/{project}_cfg_full_text_files.json'))
    ...
    print(len(all_data))
    output_file = open(f'./data/{project}_full_data_with_slices.json', 'w+')
    json.dump(all_data, output_file)
    output_file.close()
    return all_data

def extract_graph_data(portion, 
    project, base_dir='./data/output',
...
def extract_line_graph_data(
    ...
    split_dir = f'./data/{project}/raw_code/'
    parsed = f'./data/{project}/parsed/'
    wv_path = './data/gen_test/raw_code_gen_test.100'
...
def main():
    PROJECT = "gen_test"
    get_full_text_files.main(PROJECT)
    word2vec_code.main()
    extract_slices(PROJECT)
    print("done function 1\n")
    word2vec_tokenized.main()
    # assert 0
