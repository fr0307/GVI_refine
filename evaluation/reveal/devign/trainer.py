        ...
        for step_count in range(max_steps):
            ...
            if step_count % dev_every == (dev_every - 1):
                ...
                acc, pr, rc, f1 = evaluate_metrics(model, loss_function, dataset.initialize_train_batch(),
                                                   dataset.get_next_train_batch)
                debug('Step %d\tTest Accuracy:  %6.3f \t Test Precision:  %6.3f \t Test Recall: %6.3f \t Test F1:  %5.2f' % (
    ...
    try:
        for step_count in range(max_steps):
            ...
            if step_count % dev_every == (dev_every - 1):
                ...
                acc, pr, rc, f1 = evaluate_metrics(model, loss_function, dataset.initialize_test_batch(),
                                                   dataset.get_next_test_batch)
                debug('Step %d\tTest Precision:  %6.3f \t Test Recall: %6.3f \t Test F1:  %5.2f' % (step_count, pr, rc, f1))
...
def train_valid(model, dataset, max_steps, dev_every, loss_function, optimizer, save_path, log_every=50, max_patience=5):
    debug('Start Training')
    max_steps = 1000000
    train_losses = []
    best_model = None
...
def train(model, dataset, max_steps, dev_every, loss_function, optimizer, save_path, log_every, max_patience, train_mode):
    if train_mode == 'step_2000':
        ...
        return train_20(model, dataset, max_steps, dev_every, loss_function, optimizer, save_path, log_every, max_patience)
    elif train_mode == 'patience_20_valid':
        return train_valid(model, dataset, max_steps, dev_every, loss_function, optimizer, save_path, log_every, max_patience)
    else:
        raise NotImplementedError('Unknown training mode %s' % train_mode)
    
    
def train_2000_ori(model, dataset, max_steps, dev_every, loss_function, optimizer, save_path, log_every=50, max_patience=5):
    debug('Start Training')
    train_losses = []
    best_model = None
    patience_counter = 0
