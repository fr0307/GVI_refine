...
from trainer import train, predict, predict_proba, predict_proba2, evaluate as evaluate_from_model
from sam import SAM
...
class RepresentationLearningModel(BaseEstimator):
    def __init__(self,
                 ...
                 num_epoch=30, max_patience=20,  # Training Parameters
    ...
    def trainx(self, train_x, train_y, saved_model = None):
    ...
    def train(self, train_x, train_y, test1_x, test1_y, test2_x, test2_y, model_dir, saved_model = None):
        input_dim = train_x.shape[1]
        ...
        self.optimizer = Adam(self.model.parameters())
        # rho = 0
        # self.optimizer = SAM(base_optimizer=Adam, rho=rho, params=self.model.parameters())
        # print("rho: ", rho)
        if self.cuda:
            self.model.cuda(device=0)
        self.dataset = DataSet(self.batch_size, train_x.shape[1])
        # print(self.dataset.train_entries)
        # assert 0
        for _x, _y in zip(train_x, train_y):
            ...
            # else:
            #     self.dataset.add_data_entry(_x.tolist(), _y.item(), 'train')
        for _x, _y in zip(test1_x, test1_y):
            self.dataset.add_data_entry(_x.tolist(), _y.item(), 'test1')
        for _x, _y in zip(test2_x, test2_y):
            self.dataset.add_data_entry(_x.tolist(), _y.item(), 'test2')

        # print(len(self.dataset.train_entries), len(self.dataset.valid_entries))
        # print("balance", self.balance, file=self.output_buffer)
        self.dataset.initialize_dataset(balance=self.balance, output_buffer=self.output_buffer)
        # print(len(self.dataset.train_entries), len(self.dataset.valid_entries))
        # assert 0
        train(
    ...
    def predict_proba(self, text_x):
        ...
        for _x in text_x:
            self.dataset.add_data_entry(_x.tolist(), 0, part='test')
        return predict_proba2(self.model, self.dataset, cuda_device=0 if self.cuda else -1)
        #return predict_proba(
        #    model=self.model, iterator_function=self.dataset.get_next_test_batch,
        #    _batch_count=self.dataset.initialize_test_batches(), cuda_device=0 if self.cuda else -1
        #)

    def evaluate(self, text_x, test_y, neg_metrics=[], avg_metrics=[], other_metrics=[]):
        if not hasattr(self, 'dataset'):
            raise ValueError('Cannnot call predict or evaluate in untrained model. Train First!')
        self.dataset.clear_test_set()
        ...
        acc, pr, rc, f1 = evaluate_from_model(
            ...
            _batch_count=self.dataset.initialize_test_batches(), cuda_device=0 if self.cuda else -1,
            output_buffer=self.output_buffer,
            neg_metrics=neg_metrics, avg_metrics=avg_metrics, other_metrics=other_metrics
        )
        return {
            'accuracy': acc,
            'precision': pr,
