...
class DataSet:
    def __init__(self, batch_size, hdim):
        ...
        self.test1_batch_indices = []
        self.test2_batch_indices = []
    ...
    def initialize_dataset(self, balance=True, output_buffer=sys.stderr):
        if output_buffer is not None:
            print("balance: ", balance, file=output_buffer)
        if isinstance(balance, bool) and balance:
            # print('smote', file=output_buffer)
            # entries = []
            # train_features = []
            # train_targets = []
            ...
            #
            # num_positive = np.sum(train_targets == 1)
            #
            # # smote = SMOTE(sampling_strategy={1: num_positive + 1267}, random_state=1000)
            # smote = RandomOverSampler(sampling_strategy={1: num_positive + 5000}, random_state=int(balance))
        ...
        #         )
        #     positive_entries = [entry for entry in self.train_entries if entry.is_positive()]
        #     for index in range(5000):
        #         entries.append(
        #             DataEntry(self, positive_entries[index%len(positive_entries)].features, positive_entries[index%len(positive_entries)].label, positive_entries[index%len(positive_entries)].meta_data)
        #         )
            # random.seed(132492)
            # extra_entries = random.choices(positive_entries, k=5000)
            # for extra_entry in extra_entries:
            #     entries.append(
            #         DataEntry(self, extra_entry.features, extra_entry.label, extra_entry.meta_data)
            #     )
            # for _ in range(5000):
            #     extra_entry = random.choice(positive_entries)
            #     entries.append(
            #         DataEntry(self, extra_entry.features, extra_entry.label, extra_entry.meta_data)
            #     )
        ...
        self.initialize_test1_batches()
        if output_buffer is not None:
            print('Number of Test1 Entries %d #Batches %d' % \
                  (len(self.test1_entries), len(self.test1_batch_indices)), file=output_buffer)
        self.initialize_test2_batches()
        if output_buffer is not None:
            print('Number of Test2 Entries %d #Batches %d' % \
                  (len(self.test2_entries), len(self.test2_batch_indices)), file=output_buffer)
            

    def add_data_entry(self, feature, label, part='train'):
        assert part in ['train', 'valid', 'test', 'test1', 'test2']
        ...
        if part == 'train':
            ...
            self.test1_entries.append(entry)
        elif part == 'test2':
            self.test2_entries.append(entry)
    ...
    def initialize_test1_batches(self, batch_size=-1):
        if batch_size == -1:
            batch_size = self.batch_size
        self.test1_batch_indices = self.create_batches(batch_size, self.test1_entries)
        return len(self.test1_batch_indices)
        pass
    
    def initialize_test2_batches(self, batch_size=-1):
        if batch_size == -1:
            batch_size = self.batch_size
        self.test2_batch_indices = self.create_batches(batch_size, self.test2_entries)
        return len(self.test2_batch_indices)
        pass
    ...
    def get_next_test1_batch(self):
        if len(self.test1_batch_indices) > 0:
            indices = self.test1_batch_indices.pop()
            return self.prepare_data(self.test1_entries, indices)
        raise ValueError('Initialize Test1 Batch First by calling dataset.initialize_test1_batches()')
        pass
    
    def get_next_test2_batch(self):
        if len(self.test2_batch_indices) > 0:
            indices = self.test2_batch_indices.pop()
            return self.prepare_data(self.test2_entries, indices)
        raise ValueError('Initialize Test2 Batch First by calling dataset.initialize_test2_batches()')
        pass
